...
const Color colorPlum          = Color(0xFF231833);  // Done overlay background ONLY — sacred, nowhere else
...
| Plum deep rename + brand correction | v10.5 had stored `--plum` as 2D1F4A (the wrong value). The real brand deep plum from the canonical app icon SVG is `#231833` (hue 296° in OKLCH, 2° from the CTA plum hue 298° — reads as "CTA in shadow"). Token renamed to `--plum-deep` with correct value in tokens.json. `--plum` is now exclusively the PW action color (`#9B7FD4`). Global replace of the old value → `#231833` across all tracked .html / .svg / .md / .css / .dart / .json (108 files, 296 occurrences). Brand SVG `system/logo/ii-dark-on-light.svg` corrected to match `nudgii-app-icon.svg`. Verified celebration state (s05-celebration-hifi.html) renders `rgb(35, 24, 51)` via preview inspect. Rationale for choosing the canonical value: the wrong value drifted 9° toward blue-purple, breaking "one purple family" and flattening apricot warmth against the done overlay; `#231833` has tighter hue alignment with the CTA and stronger contrast (7.4:1 vs 5.9:1) with the plum-pale ii mark pill. | ✅ Yes | 2026-04-09 |
