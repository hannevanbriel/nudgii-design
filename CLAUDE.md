...
| Dashboard font migration needed | s09-today-hifi.html, s09-plan-hifi.html, s09-mystuff-hifi.html, s09-profile-hifi.html currently use Noto Serif + Manrope. Need to be migrated back to DM Serif Display + DM Sans in a dedicated session. Tonal surfaces and plum/apricot system stay unchanged. | ❌ No | — |
...
| S-05 Plan-style month headers | AHA schedule reveal (State 1) uses Plan-style layout: month headers (serif name + task count) + date column task rows. "October" and "November" groups. Dots replaced with dots as separator in subtitles. | ✅ Yes | 2026-04-06 |
