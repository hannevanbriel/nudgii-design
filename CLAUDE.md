...
| Plum deep rename | v10.5 token `--plum` renamed to `--plum-deep` to free up `--plum` as the Precision Warmth action color (`#9B7FD4`). Before the rename both v10.5 done overlay and PW action button collided on the same var name whenever a screen linked tokens.css. Post-rename: `--plum-deep` = done overlay / app icon background. `--plum` = CTA action color. No collision. | ✅ Yes | 2026-04-09 |
| Deep plum brand value: #2D1F4A (unchanged) | The brand deep plum is `#2D1F4A` — the more saturated, festive purple. Earlier in the session this was briefly changed to `#231833` based on the canonical `nudgii-app-icon.svg` value and an OKLCH hue-alignment argument. Side-by-side comparison on s05-celebration-hifi showed `#2D1F4A` reads as a "celebration" moment while `#231833` reads as solemn/funereal. The hue argument held up mathematically but didn't matter at near-black lightness where the eye can't resolve a 7° hue shift; the chroma drop (0.078 → 0.057, a 27% saturation loss) was the felt difference, and the saturated version won. Reverted across 113 files, 302 occurrences. Both brand SVGs (`system/logo/ii-dark-on-light.svg` and `system/logo/nudgii-app-icon.svg`) now unified on `#2D1F4A`. The app-icon SVG's original `#231833` value is superseded. **Rule:** the celebration moment should feel festive, not solemn. Nudgii is a reminder utility, not a serious productivity tool. | ✅ Yes | 2026-04-09 |
...
| Sub-type question pattern (MVP) | Four items require an upfront sub-type question on Add Item Review because wrong tasks destroy trust: **Car** (Petrol/Diesel/Electric/Hybrid/LPG — force pick), **Boiler** (Gas combi/Gas system/Electric/Oil — 3-layer UX: visual picker + photo help + deferred-with-universal-tasks), **Oven** (Gas/Electric — force pick with hint), **Garden pond** (Fish pond/Decorative — force pick). Scope rule: sub-type required when wrong sub-type produces wrong/misleading tasks (e.g., EV showing oil change), not by percentage of tasks that differ. Heat pump dropped from MVP (tasks overlap too much). Pattern documented in add-item-flow.html. Data model deferred to engineering (nested vs task-level applies_to array). | ✅ Yes | 2026-04-16 |
| Boiler 3-layer "Not sure" UX | Many users genuinely don't know their boiler type (renters, recent buyers, inherited homes). Layer 1: visual picker (4 cards with silhouette illustrations). Layer 2: "I don't know" → photo help (snap the info plate, Claude Vision reads it). Layer 3: "Add anyway, figure out later" → creates item with universal tasks only (annual service + filter clean) + "Setup incomplete" chip on My Stuff + one dismissible Today card (no weekly nag). When user eventually picks type, type-specific tasks get added. Pattern NOT applied to Car/Oven/Pond — users know those with near-100% confidence, escape hatches create silent failure. | ✅ Yes | 2026-04-16 |
| Vehicle mileage demoted to Details | The prominent mileage tracker card on s17-item-vehicle.html has been removed. Mileage now lives in the Details section with "last updated" honesty text, tappable to update. UX rationale: updating mileage requires physical friction (walk to car, read dashboard). Users rarely maintain accurate mileage. Dated deadlines (MOT, insurance, seasonal tyres) matter more than distance — they keep drivers legal and safe. Mileage-based tasks (oil change) work fine on yearly fallback for 90% of drivers. Countdown cards now promoted to primary position (immediately after stats). Contextual prompt may surface inside oil change task expand when task is approaching. | ✅ Yes | 2026-04-16 |
| AI sub-type detection from scan | AI scan responses now include subtype_detected + subtype_confidence alongside tasks. High confidence (&gt;0.85) pre-selects the chip. Medium (0.50-0.85) highlights but doesn't select. Low or null = normal user-picks flow. Claude Vision signals: car fuel type from charging port/fuel cap/badge, boiler type from info plate text, oven from visible burners vs elements, pond from visible fish. For unlisted items with sub-type relevance (e.g., "Aga cooker"), AI sets subtype_required: true and provides plausible options. Spec in ai-template-generation.html. | ✅ Yes | 2026-04-16 |
